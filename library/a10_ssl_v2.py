...
def writeFile(name, filename, body, boundary, writer, needsCRLF):
    crlf(writer, needsCRLF)
    block = [boundary,
        'Content-Disposition: form-data; name="file_name"; filename="%s"' % filename,
        'Content-Type: application/octet-stream'  + CRLF
    ]
    writer.write(CRLF.join(block))
    writer.write(CRLF)
    writer.write(body)
    #writer.write(CRLF)
...
def buildPayload(filename):
    ...
    try:
        ...
        try:
            ...
            writeFile("upload", name, data, field_boundary, payload, needsCRLF)
        finally:
            f.close()
    except Exception, e:
        raise e
    payload.write(CRLF + field_boundary + '--' + CRLF)
    return header_boundary, payload.getvalue()

def uploadSSL(module, url, name, filepath):
...
def main():
    ...
    if state == 'present':
        if method == "upload":
            if os.path.isfile(file_name) is False:
                ...
                result = uploadSSL(module, session_url + '&method=slb.ssl.upload&type=' +
                                file_type + '&file_name=' + os.path.basename(file_name), 'upload', file_name)
        ...
        result = axapi_call(module, session_url + '&method=slb.ssl.search', json.dumps({'file_name': os.path.basename(file_name)}))
        if ('response' in result and result['response']['status'] == 'fail'):
            module.fail_json(msg=result['response']['err']['msg'])

        result = axapi_call(module, session_url + '&method=slb.ssl.delete', json.dumps({'file_name': os.path.basename(file_name), 'type': file_type}))
        if ('response' in result and result['response']['status'] == 'fail'):
            module.fail_json(msg=result['response']['err']['msg'])
        else:
            changed = True
    ...
    result = axapi_call(module, session_url + '&method=session.close')
